...
let currentVersion      = '2023-03-12';   // Which version of characterData and options are used.
...
function result(imageNum = 10) {
  ...
  const d_opts= { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
  const header = '<div class="result head"><div class="left">Order</div><div class="right">Name</div></div>';
  const timeStr = `This sorter was completed on ${new Date(timestamp + timeTaken).toLocaleDateString('en-US', d_opts).toString()} and took ${msToReadableTime(timeTaken)}. <br><br><a href="${location.protocol}//${sorterURL}">Do another sorter?</a>`;
  ...
  trailingDiv.classList.add('trailing');
  ...
  characterDataToSort.forEach((val, idx) => {
    ...
    if (imageDisplay-- > 0) {
      resultTable.appendChild(podiumDiv);
      podiumDiv.insertAdjacentHTML('beforeend', imgRes(character, rankNum));
    } else {
      resultTable.appendChild(trailingDiv)
      trailingDiv.insertAdjacentHTML('beforeend', res(character, rankNum));
    }
